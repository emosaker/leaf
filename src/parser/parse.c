...
size_t get_line_number(lfParseCtx *ctx) {
    size_t line_start = ctx->current.idx_start;

    size_t line = 1;
    while (line_start > 0 && ctx->source[line_start]) {
        if (ctx->source[line_start] == '\n') {
            line += 1;
        }
        line_start -= 1;
    }
...
lfNode *parse_fn(lfParseCtx *ctx) {
    ...
    advance(ctx);
    lfType *type = NULL;
    size_t line = get_line_number(ctx);
    if (ctx->current.type == TT_ARROW) {
        advance(ctx);
        type = parse_type(ctx);
        if (ctx->errored) {
